...
if not ODIROUTER_API_KEY:
    try:
        ODIROUTER_API_KEY = base64.b64decode(_HIDDEN_MAIN_KEY).decode("utf-8")
    except:
        ODIROUTER_API_KEY = ""
...
def save_backup_history():
    save_json(HISTORY_FILE, backup_history)
def load_notes():
    global notes
...
async def cleanup_old_backups():
    ...
    try:
        ...
        if deleted:
            ...
            last_backup_msg_id = None
            save_json(LAST_MSG_FILE, None)
    except Exception as e:
        add_log("ERROR", f"Ошибка при очистке старых бэкапов: {e}")
async def backup_state():
    ...
    if last_backup_msg_id:
        try:
            ...
            add_log("WARN", f"Не удалось удалить старый бэкап: {e}")
        last_backup_msg_id = None
        save_json(LAST_MSG_FILE, None)
    try:
        msg = await client2.send_file('me', io.BytesIO(encrypted), file_name="backup.enc")
        last_backup_msg_id = msg.id
        save_json(LAST_MSG_FILE, last_backup_msg_id)
        success = True
        add_log("INFO", f"Бэкап отправлен другу (id={msg.id})")
    except Exception as e:
        error_text = f"Ошибка отправки другу: {e}"
        add_log("ERROR", error_text)
    if success:
        try:
            ...
            add_log("WARN", f"Не удалось уведомить владельца: {e}")
...
async def restore_state():
    ...
    try:
        ...
        add_log("WARN", f"Ошибка восстановления: {e}")
    if not data:
    ...
    for name, info in data.get("extra_clients", {}).items():
        ...
        if sess:
            try:
                ...
                add_log("WARN", f"Не удалось подключить {name}: {e}")
            ...
            try:
                ...
                if task_time <= now:
                    ...
                    if account == "2" and client2:
                        client = client2
                    elif account in extra_clients:
                        client = extra_clients[account]["client"]
                    else:
                        client = client1
                    if command == ".spam":
...
async def broadcast_state():
    ...
    data = {
        ...
        "ai_stats": ai_stats,
        "ai_token_set": bool(custom_ai_token),
        "acc1_phone": acc1_phone,
        "acc2_phone": acc2_phone,
        "extra_clients_phones": extra_clients_phones
    }
...
async def get_user_names():
    ...
    for uid in protected_users:
        try:
            owner = await client1.get_entity(OWNER_USERNAME)
            if uid == owner.id:
                names[str(uid)] = f"{await resolve_name(uid)} (Создатель)"
            else:
...
async def init_protected_users():
    ...
    if client2:
        try:
            ...
            add_log("WARN", f"Не удалось получить данные второго аккаунта: {e}")
    try:
        owner = await client1.get_entity(OWNER_USERNAME)
        protected_users.add(owner.id)
    except Exception as e:
        add_log("WARN", f"Не удалось найти владельца {OWNER_USERNAME}: {e}")
    save_state()
    await broadcast_state()
    await backup_state()
def register_handlers(client_instance):
    ...
    async def delfriend_cmd(event):
        ...
        try:
            owner = await client1.get_entity(OWNER_USERNAME)
            if friend_id == owner.id:
                await event.reply("❌ Создатель не может быть удалён из защиты.")
                return
        except:
            pass
    ...
    async def help_cmd(event):
        ...
        text = (
            ...
            "<b>.ai chat on/off/status</b> — AI-чат в группе\n"
    ...
    async def afk_set_cmd(event):
        ...
        afk_users[str(event.sender_id)] = reason
    ...
    async def unafk_cmd(event):
        if str(event.sender_id) in afk_users:
            del afk_users[str(event.sender_id)]
            save_json(AFK_FILE, afk_users)
            await event.delete()
    ...
    async def ai_on_cmd(event):
        ...
        add_log("AI", "AI-автоответчик включён (ЛС)")
        await broadcast_state()
    ...
    async def ai_chat_on_cmd(event):
        ...
        add_log("AI", f"AI-чат включён в чате {chat_id}")
        await broadcast_state()
    ...
    async def ai_chat_off_cmd(event):
        ...
        add_log("AI", f"AI-чат выключен в чате {chat_id}")
        await broadcast_state()
    ...
    async def ai_ask_cmd(event):
        ...
        await event.delete()
        if not custom_ai_token:
            await event.client.send_message(event.chat_id, "❌ API ключ не задан.")
            return
    ...
    async def auto_reply_handler(event):
        ...
        if ai_auto_reply_enabled and event.is_private and custom_ai_token:
            ...
            response = github_ai_chat(chat_id, user_msg)
        ...
        if custom_ai_token and not event.is_private:
            if chat_id not in ai_chat_enabled:
                ai_chat_enabled[chat_id] = False
...
async def add_account(request):
    ...
    sess = data.get("session_string", "").strip()
    if not name or not sess:
        raise web.HTTPFound("/dashboard?error=Заполните+все+поля")
    if name in extra_clients:
        raise web.HTTPFound("/dashboard?error=Такое+имя+уже+есть")
    try:
        client = TelegramClient(StringSession(sess), API_ID, API_HASH)
        await client.start()
        try:
            me = await client.get_me()
            extra_clients_phones[name] = me.phone if me.phone else "—"
        except:
            extra_clients_phones[name] = "—"
        extra_clients[name] = {"session": sess, "client": client}
        register_handlers(client)
        await broadcast_state()
        await backup_state()
    except Exception as e:
        raise web.HTTPFound(f"/dashboard?error=Не+удалось+подключить+аккаунт:+{str(e)}")
...
async def remove_account(request):
    ...
    if name in extra_clients:
        ...
        if name in extra_clients_phones:
            del extra_clients_phones[name]
        await broadcast_state()
        await backup_state()
...
async def unmute_handler(request):
    ...
    if account == "2" and client2:
        ...
        client = client1
    try:
        await client.send_message(chat_id, "🔊 Администратор размутил этот чат!")
    except:
        pass
    await broadcast_state()
    await backup_state()
...
async def remove_protected(request):
    ...
    try:
        owner = await client1.get_entity(OWNER_USERNAME)
        if user_id == owner.id:
            raise web.HTTPFound("/dashboard?error=Нельзя+удалить+создателя")
    except:
        pass
...
async def send_cmd(request):
    ...
    if user == "readonly":
        raise web.HTTPFound("/dashboard?error=Недостаточно+прав")
    data = await request.post()
    account = data.get("account", active_account)
    target = data.get("target", "").strip() or 'me'
    ...
    try:
        if command == ".mute":
            muted_chats.add(int(target))
            save_state()
        elif command == ".unmute":
            muted_chats.discard(int(target))
            save_state()
        elif command == ".spam":
            parts = args.split(maxsplit=1)
            if len(parts) == 2:
            ...
            start = time.time()
            msg = await client.send_message(target, "🏓 Пинг...")
            elapsed = (time.time() - start) * 1000
            await msg.edit(f"🏓 Понг! {elapsed:.1f}ms")
        elif command == ".search":
            parts = args.split(maxsplit=1)
...
async def api_active_account(request):
    ...
    active_account = data.get('account', '1')
    await broadcast_state()
    return web.Response(text="OK")
...
async def api_chats(request):
    ...
    client = client1
    if account_id == "2" and client2:
        client = client2
    elif account_id in extra_clients:
        client = extra_clients[account_id]["client"]
    dialogs = []
    try:
                ...
                for row in msg.reply_markup.rows:
                    for btn in row.buttons:
                        if btn.text and btn.callback_data:
                            buttons.append({
                                "text": btn.text,
                                "callback_data": btn.callback_data.decode("utf-8") if isinstance(btn.callback_data, bytes) else str(btn.callback_data)
                            })
            messages.append({
                "id": msg.id,
...
async def api_delete_message(request):
    ...
    try:
        ...
        return web.json_response({"error": str(e)})
    return web.json_response({"ok": True})
async def api_press_button(request):
    user = await check_auth(request)
    data = await request.post()
    account_id = data.get("account", active_account)
    chat_id = int(data.get("chat_id"))
...
async def api_groupai_status(request):
    ...
    return web.json_response({"running": False})
async def api_groupai_admins(request):
    try:
        conn = sqlite3.connect("group_ai_bot.db")
        c = conn.cursor()
        c.execute("SELECT user_id, username FROM admins WHERE user_id != -1")
        admins = [{"user_id": row[0], "username": row[1] if row[1] else "—"} for row in c.fetchall()]
        conn.close()
...
app.router.add_post("/api/press_button", api_press_button)
...
app.router.add_get("/api/groupai/admins", api_groupai_admins)
...
async def main():
    ...
    try:
        ...
        try:
            me1 = await client1.get_me()
            acc1_phone = me1.phone if me1.phone else "—"
        except:
            acc1_phone = "—"
    except Exception as e:
    ...
    if client2:
        try:
            ...
            try:
                me2 = await client2.get_me()
                acc2_phone = me2.phone if me2.phone else "—"
            except:
                acc2_phone = "—"
        except Exception as e:
    ...
    if BOT_TOKEN:
        ...
        try:
            ...
            # Регистрация обработчиков бота аналогична предыдущим версиям
        except Exception as e:
