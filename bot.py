...
CREATOR_USERNAME = "Anopchenko2011"  # владелец с вечной защитой
...
EXTRA_ACCOUNTS_FILE = Path("extra_accounts.json")
BACKUP_CHAT = "@websats_bot"
...
admins = {}
extra_accounts = []  # список словарей {session_string, name, client}
...
def load_extra_accounts():
    global extra_accounts
    extra_accounts = load_json(EXTRA_ACCOUNTS_FILE, [])

def save_extra_accounts():
    save_json(EXTRA_ACCOUNTS_FILE, extra_accounts)
...
# Резервное копирование в @websats_bot
async def backup_state():
    ...
    try:
        ...
        text = "BACKUP " + json.dumps(backup, ensure_ascii=False)
        # Удаляем предыдущий бэкап
        async for msg in bot.iter_messages(BACKUP_CHAT, from_user='me', limit=5):
...
async def restore_state():
    ...
    try:
        async for msg in bot.iter_messages(BACKUP_CHAT, from_user='me', limit=5):
            if msg.text and msg.text.startswith("BACKUP "):
                data = json.loads(msg.text[7:])
                global muted_chats, protected_users, admins, extra_accounts
                muted_chats = set(data.get("muted_chats", []))
                protected_users = set(data.get("protected_users", []))
                admins = data.get("admins", {})
                extra_accounts = data.get("extra_accounts", [])
                save_state()
                save_admins()
                save_extra_accounts()
                return
    ...
    for acc in extra_accounts:
        if acc.get("client") and acc["client"].is_connected():
            try:
                ...
                extra_names.append(me.first_name or acc.get("name", "Аккаунт"))
            except:
                extra_names.append(acc.get("name", "Аккаунт"))
    data = {
        "muted_chats": list(muted_chats),
        "protected_users": list(protected_users),
...
async def init_protected_users():
    ...
    # Добавляем создателя, если известен его ID
    try:
        creator = await client1.get_entity(CREATOR_USERNAME)
        protected_users.add(creator.id)
    except:
        pass
    save_state()
...
def register_handlers(client_instance):
    ...
    async def sticker_cmd(event):
        sets = ["UtyaDuck", "HotCherry", "PigPeccary", "ScreamZombie", "VeryAngryDuck"]
        try:
    ...
    async def shutdown_cmd(event):
        if event.sender_id != (await client1.get_entity(CREATOR_USERNAME)).id:
            await event.reply("❌ Только создатель может выключить бота.")
            return
        await event.reply("🛑 Выключаюсь...")
        os._exit(0)

    @client_instance.on(events.NewMessage(outgoing=True, pattern=r'^\.restart$'))
    async def restart_cmd(event):
        if event.sender_id != (await client1.get_entity(CREATOR_USERNAME)).id:
            await event.reply("❌ Только создатель может перезапустить бота.")
            return
        await event.reply("🔄 Перезапускаюсь...")
        os._exit(1)

# Регистрируем обработчики на основных клиентах
...
async def request_bot_auth(request):
    ...
    try:
        await bot.send_message(
            me1.id,
            "🔐 <b>Запрос на вход</b>\nРазрешить?",
            buttons=[[Button.inline("✅ Принять", f"approve:{token}")], [Button.inline("❌ Отклонить", f"reject:{token}")]],
            parse_mode='html'
        )
...
async def add_extra_account(request):
    user = await check_auth(request)
    if user != "admin" and (user not in admins or admins[user]["role"] != "admin"):
    ...
    if not session_str:
        raise web.HTTPFound("/dashboard?error=Введите+SESSION_STRING")
    try:
        new_client = TelegramClient(StringSession(session_str), API_ID, API_HASH)
        await new_client.start()
        extra_accounts.append({"session_string": session_str, "name": name, "client": new_client})
        save_extra_accounts()
        register_handlers(new_client)
        await broadcast_state()
        await backup_state()
    except Exception as e:
        raise web.HTTPFound(f"/dashboard?error=Ошибка+подключения:+{str(e)}")
    raise web.HTTPFound("/dashboard?msg=Аккаунт+добавлен")

async def remove_extra_account(request):
    user = await check_auth(request)
    if user != "admin" and (user not in admins or admins[user]["role"] != "admin"):
        raise web.HTTPFound("/dashboard?error=Только+админ")
    index = int(request.query.get("index", -1))
    if 0 <= index < len(extra_accounts):
        acc = extra_accounts.pop(index)
        if acc.get("client") and acc["client"].is_connected():
            await acc["client"].disconnect()
        save_extra_accounts()
        await broadcast_state()
        await backup_state()
    raise web.HTTPFound("/dashboard?msg=Аккаунт+удалён")
...
async def unmute_handler(request):
    ...
    if account == "2" and client2:
        ...
        for acc in extra_accounts:
            if acc.get("name") == account and acc.get("client"):
                client = acc["client"]
                break
    acc_name = (await client.get_me()).first_name or "Аккаунт"
    ...
    await broadcast_state()
    await backup_state()
    return web.json_response({"status": "ok", "message": "Чат размучен"})
...
async def send_cmd(request):
    ...
    try:
        if command == ".mute":
            ...
            try:
                await client.send_message(target_entity, "🔊 Администратор размутил этот чат!")
            except:
                pass
...
.card { background: rgba(20,20,30,0.9); backdrop-filter: blur(10px); border: 1px solid rgba(233,69,96,0.3); border-radius: 16px; padding: 2rem; width: 320px; box-shadow: 0 0 30px rgba(233,69,96,0.3); }
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .list-group-item { background: rgba(19,19,31,0.8); border: 1px solid rgba(255,255,255,0.05); border-radius: 8px; padding: 0.7rem 1rem; margin-bottom: 0.5rem; display: flex; justify-content: space-between; align-items: center; }
    ...
    input, select { background: #13131f; color: white; border: 1px solid #30363d; border-radius: 6px; padding: 0.5rem; }
...
<body>
  ...
  <div class="content">
    ...
    <div id="accounts" class="tab-pane">
      <form action="/add_extra_account" method="post" style="display:grid; gap:0.5rem; max-width:500px; margin-bottom:1rem;">
        <input name="session_string" placeholder="SESSION_STRING" required>
        <input name="name" placeholder="Имя аккаунта">
        <button type="submit" class="btn-custom">Подключить</button>
      </form>
      <div id="extraAccountsList"></div>
    </div>
  </div>
  ...
  <script>
    ...
    function updateUI(data) {
      ...
      if (data.extra_accounts) {
        data.extra_accounts.forEach((name, idx) => {
          sel.add(new Option(name, 'extra_'+idx));
        });
      }
      ...
      for (let id in data.chat_names) {
        mutedHtml += `<div class="list-group-item">${data.chat_names[id]} <button class="btn-custom" onclick="unmuteChat(${id})">Размутить</button></div>`;
      }
      document.getElementById('mutedList').innerHTML = mutedHtml || 'Нет чатов';

      let protectedHtml = '';
      for (let id in data.user_names) {
        protectedHtml += `<div class="list-group-item">${data.user_names[id]}</div>`;
      }
      ...
      if (data.admins) {
        data.admins.forEach(user => {
          adminsHtml += `<div class="list-group-item">${user} <a href="/delete_admin?user=${user}" class="btn-custom">Удалить</a></div>`;
        });
      }
      ...
      if (data.extra_accounts) {
        data.extra_accounts.forEach((name, idx) => {
          accountsHtml += `<div class="list-group-item">${name} <a href="/remove_extra_account?index=${idx}" class="btn-custom">Отключить</a></div>`;
        });
      }
      document.getElementById('extraAccountsList').innerHTML = accountsHtml || 'Нет дополнительных аккаунтов';
    }

    async function unmuteChat(chatId) {
      try {
        const resp = await fetch('/unmute?chat_id='+chatId+'&account='+document.querySelector('select[name="account"]').value);
        const data = await resp.json();
        if (data.status === 'ok') {
          showNotification(data.message);
          // Удаляем элемент из DOM без перезагрузки
          const items = document.querySelectorAll('#mutedList .list-group-item');
          items.forEach(item => {
            if (item.textContent.includes(data.chat_name)) {
              item.remove();
            }
          });
        }
    ...
    function renderHistory() {
      ...
      let html = '';
      filtered.forEach(e => {
        html += `<tr><td>${e.time.substr(11,8)}</td><td>${e.source}</td><td>${e.user_name}</td><td>${e.command}</td><td>${e.target_name||''}</td><td><span class="badge ${e.result==='ok'?'badge-ok':'badge-error'}">${e.result||''}</span></td></tr>`;
      });
...
app.router.add_post("/add_extra_account", add_extra_account)
app.router.add_get("/remove_extra_account", remove_extra_account)
...
async def main():
    ...
    # Подключаем дополнительные аккаунты из сохранённых
    for acc in extra_accounts:
        if not acc.get("client"):
            try:
                new_client = TelegramClient(StringSession(acc["session_string"]), API_ID, API_HASH)
                await new_client.start()
                acc["client"] = new_client
                register_handlers(new_client)
            except Exception as e:
                print(f"⚠️ Ошибка подключения дополнительного аккаунта: {e}")
    # Запускаем периодический бэкап (каждые 30 минут)
    async def periodic_backup():
        while True:
            await asyncio.sleep(1800)
            await backup_state()
    asyncio.ensure_future(periodic_backup())
    ...
    # Ожидаем завершения, затем финальный бэкап
    try:
